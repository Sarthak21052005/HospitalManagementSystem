...
-- Production Schema v2.0 with Billing & Bug Fixes
-- ========================================
...
DROP INDEX IF EXISTS idx_payment_transaction_bill CASCADE;
DROP INDEX IF EXISTS idx_payment_transaction_date CASCADE;
...
-- ✅ UPDATED: Task table with UNIQUE constraint to prevent duplicates
CREATE TABLE Task (
    ...
    -- ✅ NEW: Prevent duplicate tasks for same record
    CONSTRAINT unique_task_per_record UNIQUE (record_id, task_type)
);
...
-- ✅ NEW: Payment Transaction table for tracking payments
CREATE TABLE Payment_Transaction (
    transaction_id SERIAL PRIMARY KEY,
    bill_id INT REFERENCES Bill(bill_id) ON DELETE CASCADE,
    amount DECIMAL(10, 2) NOT NULL CHECK (amount > 0),
    payment_method VARCHAR(20) NOT NULL CHECK (payment_method IN ('Cash', 'Card', 'UPI', 'Bank Transfer', 'Insurance')),
...
CREATE INDEX idx_payment_transaction_bill ON Payment_Transaction(bill_id);
...
-- ✅ UPDATED: Task creation trigger with duplicate prevention
CREATE OR REPLACE FUNCTION create_task_on_medical_record()
RETURNS TRIGGER AS $$
BEGIN
    -- ✅ Only create task if one doesn't already exist for this record
    IF NOT EXISTS (
        SELECT 1 FROM Task 
        WHERE record_id = NEW.record_id 
        AND task_type = 'REVIEW_REPORT'
    ) THEN
        ...
        VALUES (
            ...
            'REVIEW_REPORT',
            CASE 
                WHEN (SELECT is_serious_case FROM Patient WHERE patient_id = NEW.patient_id) 
                THEN 'URGENT' 
                ELSE 'ROUTINE' 
            END,
            'PENDING',
...
SELECT '💰 Billing system with Payment_Transaction ready!' as billing;
SELECT '🐛 Duplicate task bug FIXED!' as bug_fixes;
